...
sealed class DiagnosticBag : IEnumerable<Diagnostic>
{
    ...
    public void ReportUnexpectedToken(SyntaxToken foundToken, SyntaxKind expected) =>
        diagnostics.Add(new(Diagnostic.SYX0000, foundToken.TextSpan, $"Unexpected {foundToken.Kind} ('{foundToken.Text}'), expected {expected}."));
    
    public void ReportUnaryOperatorTypeMismatch(SyntaxToken operatorToken, Type type) =>
        diagnostics.Add(new(Diagnostic.BND0000, operatorToken.TextSpan, $"Unary operator '{operatorToken.Text}' cannot be applied to type '{type}'."));
    public void ReportBinaryOperatorTypeMismatch(SyntaxToken operatorToken, Type left, Type right) => diagnostics.Add(
        new(Diagnostic.BND0001, operatorToken.TextSpan, $"Binary operator '{operatorToken.Text}' cannot be applied to types '{left}' and '{right}'."));
