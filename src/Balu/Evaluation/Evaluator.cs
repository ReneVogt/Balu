...
sealed class Evaluator : BoundTreeVisitor
{
    ...
    protected override BoundNode VisitBoundBinaryExpression(BoundBinaryExpression binaryExpression)
    {
        ...
        Result = binaryExpression.Operator.OperatorKind switch
        {
            ...
            BoundBinaryOperatorKind.BitwiseAnd => left is bool l ? l && (bool)right : (int)left & (int)right,
            BoundBinaryOperatorKind.BitwiseOr => left is bool l ? l || (bool)right : (int)left | (int)right,
            BoundBinaryOperatorKind.BitwiseXor => left is bool l ? l ^ (bool)right : (int)left ^ (int)right,
