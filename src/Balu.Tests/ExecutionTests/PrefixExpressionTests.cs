...
{
    [Fact]
    public void Script_PrefixExpression_ReportsUndefinedName() => " ++[abc]".AssertScriptEvaluation("BL1017: Undefined variable 'abc'.");
    [Theory]
    [InlineData("{ let abc = 12 ++[abc] }", "BL1010: Variable 'abc' is readonly and cannot be assigned to.")]
    [InlineData("{ var abc = true [--abc] }", "BL1003: Prefix operator '--' cannot be applied to type 'bool'.")]
    [InlineData("{ var abc = \"\" [++abc] }", "BL1003: Prefix operator '++' cannot be applied to type 'string'.")]
